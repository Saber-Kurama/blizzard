var Home = {
  ...
  getNews: function() {
    Common.getData("/action/home/main-body", "",
        function(a) {
          $(".news .g-wrap-limit").html(a),
              Home.getMoreNews()
        })
  },
  getMoreNews: function() {
    var a = $(".morenews"),
        b = $(".show-for-medium-down").data("totalpage"),
        c = 1;
    1 == b ? a.hide() : a.show(),
        a.find("a").live("click",
            function() {
              ...
              a.find("span").html("更多消息"),
                  Common.getData(b, "",
                      function(b) {
                        a.find("span").html("加载中");
                        var d = juicer(Home.setMdHtml(), b),
                            e = juicer(Home.setLgHtml(), b);
                        $(".news-small-list").append(d),
                            $(".hide-for-medium-down .pure-g").append(e),
                            c == b.totalPage ? a.html("<span class='more-btn'>已无更多新闻</span>") : a.find("span").html("更多消息")
                      },
                      "json")
            })
  },
  setMdHtml: function() {
    ...
    return juicer.register("formatDate", Common.formatDate),
        a
  },
  setLgHtml: function() {
  ...
  heroSelect: function() {
    var a = $(".chars.section"),
        b = $("#character-overlay"),
        c = $("#character-overlay-text"),
        d = $("#character-overlay-text-border"),
        e = $("#character-overlay-wrap"),
        f = $(".selection-wrap"),
        g = $(".pure-menu"),
        h = "overlay-loaded";
    f.on("mouseenter", ".selection-link",
        function() {
          var f = $(this),
              i = f.data("hero-slug"),
              j = f.data("hero-role"),
              k = f.data("hero-role-id"),
              l = f.data("hero-overlay");
          g.hasClass("menu-hidden") || g.addClass("menu-hidden"),
              Home.timer = setTimeout(function() {
                    e.addClass("loading"),
                        $.imgpreload(l, {
                          all: function() {
                            e.removeClass("loading"),
                                a.addClass("overlay-ready overlay-loaded"),
                                b.attr("style", "background-image:url(" + l + ")"),
                                d.addClass(h),
                                c.addClass(h).find("h2").html(i).end().find("i").attr("class", "heroes-icon role-" + k).end().find("p").text(j)
                          }
                        })
                  },
                  100)
        }),
        f.on("mouseleave", ".selection-link",
            function() {
  ...
  setProm: function(a) {
    ...
    else var b = ['<div class="promo overlay" overlay="promo">', '<div class="inner">', '<div class="close x"></div>', '<div class="top">', '<div class="actions">', '<div class="box">', "<h1>新英雄</h1>", "<h2>" + a.newHero.name1 + "</h2>", '<a href="' + a.newHero.href1 + '" class="btn"><span>查看英雄</span></a>', "</div>", "</div>", '<img class="img basic" src="' + a.newHero.src + '" />', "</div>", '<div class="btm">', '<div class="text"><a href="/heroes/" tartget="_blank">查看更多其他英雄</a></div>', "</div>", "</div>", '<div class="blackout close"></div>', "</div>"].join("");
    var c = a.newHero.showProm;
    Home.setCookies(c, b)
  },
  setHomeProm: function() {
    var a = ['<div class="promo overlay" overlay="promo">', '<div class="inner">', '<div class="close x"></div>', '<div class="top">', '<a href="javascript:void(0);" class="showVideo"><img class="img" src="http://overwatch.nos.netease.com/1/images/home/pop_hgf.jpg" /></a>', "</div>", "</div>", '<div class="blackout close"></div>', "</div>"].join(""),
        b = !0;
    Home.setCookies(b, a)
  },
  setVideoHtml: function(a, b, c, d) {
    if (Modernizr.video) var e = ['<video controls autoplay preload="auto">', '<source src="' + a + '" type="video/mp4">', "您的浏览器暂时无法播放此视频.</video>"].join("");
    else var f = "http://v.163.com/swf/video/NetEaseFlvPlayerV3.swf",
        e = ['<object width="' + c + '" height="' + d + '" id="FPlayer" data="' + f + '" type="application/x-shockwave-flash">', '<param value="transparent" name="wmode" />', '<param value="true" name="allowFullScreen" />', '<param value="always" name="allowscriptaccess" />', '<param value="' + f + '" allownetworking="all" name="movie" />', '<param value="' + b + '" name="flashvars" />', "</object>"].join("");
    return e
  },
  showPromVideo: function() {
    ...
    a.on("click",
        function() {
          ...
          $(this).parent(".top").html('<div class="videoWrap">' + a + "</div>"),
              $(".videoWrap video").css({
                width: "100%",
                height: "100%"
              }),
  ...
  setCookies: function(a, b) {
    ...
    a && ($.cookie("prom_buy_cookie") || ($("body").append(b), Home.showPromVideo(), setTimeout(function() {
        ...
        200), $.cookie("prom_buy_cookie", "prom", {
      expires: 2
    })), $(".blackout,.close").on("click", c).on("click", ".inner",
        function(a) {
          a.stopPropagation()
        }))
  }
